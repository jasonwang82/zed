...
use project::agent_server_store::{AllAgentServersSettings, CODEBUDDY_NAME};
...
impl AgentServer for CodeBuddy {
    ...
    fn connect(
        ...
        cx.spawn(async move |cx| {
            let (command, root_dir, login) = store
                .update(cx, |store, cx| {
                    let agent = store
                        .get_external_agent(&CODEBUDDY_NAME.into())
                        .context("CodeBuddy is not registered")?;
